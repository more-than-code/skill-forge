...
6. If install is required, pass **both** `--path` and `--yes`. `--yes` only skips overwrite
   confirmation; without `--path` the CLI prompts interactively for the target path and will
   **hang** with stdin closed. Default skill dirs by tool:

```bash
# codex
skf install <name> --type skill --target codex --path ~/.codex/skills --yes
# claude-code
