...
/**
 * Resolve a registry skill path and ensure it stays under inventory/skills.
 * @returns {string} absolute resolved path
 */
export function assertPathWithinInventorySkills(relPath, repoRoot) {
  if (path.isAbsolute(relPath)) throw new Error(`Registry entry has an absolute path "${relPath}"; refusing to delete.`);
  ...
  if (resolved !== boundary && !resolved.startsWith(`${boundary}${path.sep}`)) {
    throw new Error(`Registry entry path "${relPath}" escapes inventory/skills; refusing to delete.`);
  }
  return resolved;
}
